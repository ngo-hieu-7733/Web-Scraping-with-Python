...
class BookSpiderTest(unittest.TestCase):
    ...
    def test_parse_creates_pagination_request(self):
        ...
        next_page_req = results[-1]

        self.assertIsInstance(next_page_req, Request)
        self.assertEqual(next_page_req.url, "https://books.toscrape.com/catalogue/page-2.html")

if __name__ == "__main__":
    """
    The .main() will:
    - first, find all classes that inherit TestCase class
    - then run all methods whose name starts with "test_"
    - finally, log results: pass/fail/error
    """
    unittest.main()